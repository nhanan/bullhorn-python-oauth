...
auth_url = "https://auth.bullhornstaffing.com/" # May need to change these depending on where you're based - see notes above
...
def getAccessToken():
    ...
    url = f"{auth_url}oauth/token?grant_type=authorization_code&code={authCode}&client_id={client_id}&client_secret={client_secret}"
    response = requests.request("POST", url)
    accessToken = response.json()["access_token"]
    return(accessToken)

def getAuthCode():
    url = f"{auth_url}oauth/authorize?client_id={client_id}&response_type=code&action=Login&username={api_user}&password={api_pass}"
